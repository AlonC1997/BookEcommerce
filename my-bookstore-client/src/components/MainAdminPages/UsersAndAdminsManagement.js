...
const UsersAndAdminsManagement = () => {
	 ...
	 * - Uppercase letter
	 * - Passwords match
	 * @returns {string} - The error message if the password does not meet the criteria, or an empty string if it is valid.
	 */
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.filters}>
				<input type="text" placeholder="Search by ID, Name, Username, or Role" value={searchTerm} onChange={handleSearchChange} className={styles.searchBox} />
				<div className={styles.checkboxes}>
					<label>
						<input type="checkbox" name="MAIN_ADMIN" checked={tableFilters.MAIN_ADMIN} onChange={handleFilterChange} />
						MAIN_ADMIN
					</label>
					<label>
						<input type="checkbox" name="ADMIN" checked={tableFilters.ADMIN} onChange={handleFilterChange} />
						ADMIN
					</label>
					<label>
						<input type="checkbox" name="USER" checked={tableFilters.USER} onChange={handleFilterChange} />
						USER
					</label>
				</div>
				<button onClick={() => setShowAddForm((prev) => !prev)} className={showAddForm ? styles.cancelButtonOut : styles.addButtonOut}>
				...
				<tbody>
					{filteredUsers.map((user) => (
						<tr key={user.userID}>
							...
							<td>{user.role}</td>
							<td>
								<button onClick={() => handleDeleteUser(user.userID)} disabled={user.role === 'MAIN_ADMIN'} className={styles.deleteButton}>
									Delete
								</button>
							</td>
						</tr>
				...
				<div className={styles.formContainer}>
					...
					<form onSubmit={handleAddUser} className={styles.form}>
						<div className={styles.formGroup}>
							...
							<input type="text" id="username" name="username" value={newUser.username} onChange={handleInputChange} required />
						...
						<div className={styles.formGroup}>
							...
							<input type="password" id="passwordAgain" name="passwordAgain" value={newUser.passwordAgain} onChange={handleInputChange} required />
							<PasswordChecklist
						...
						<div className={styles.formGroup}>
							...
							<input type="text" id="name" name="name" value={newUser.name} onChange={handleInputChange} required />
						</div>
						<div className={styles.formGroup}>
							<label htmlFor="address">Address:</label>
							<input type="text" id="address" name="address" value={newUser.address} onChange={handleInputChange} required />
						</div>
						<div className={styles.formGroup}>
