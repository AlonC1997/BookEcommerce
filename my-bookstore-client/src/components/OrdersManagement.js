...
const OrderManagement = () => {
    ...
    const [userDetails, setUserDetails] = useState({ id: '', name: '', address: '' });
    ...
    const fetchUserDetails = async (userId) => {
        try {
            ...
            setUserDetails({ id, name, address });
    ...
    const handleInputChange = (e, orderId, field) => {
        const updatedOrders = orders.map(order => 
            order.id === orderId ? { ...order, [field]: e.target.value } : order
        );
        ...
        <div className={styles.container}>
                ...
                <div className={styles.checkboxContainer}>
                    <label className={styles.checkboxLabel}>
                        <input
                            type="checkbox"
                            name="INPROCESS"
                            checked={selectedStatuses.INPROCESS}
                    ...
                    <label className={styles.checkboxLabel}>
                        <input
                            type="checkbox"
                            name="SHIPPED"
                            checked={selectedStatuses.SHIPPED}
                            onChange={handleStatusChange}
                        />
                        SHIPPED
                    </label>
                    <label className={styles.checkboxLabel}>
                        <input
                            type="checkbox"
                            name="DELIVERED"
                            checked={selectedStatuses.DELIVERED}
                            onChange={handleStatusChange}
                        />
                        DELIVERED
                    </label>
                        ...
                        <tr key={order.id}>
                            ...
                            <td className={styles.td}>
                                <input 
                                    ...
                                    onChange={(e) => handleInputChange(e, order.id, 'totalPrice')}
                            ...
                            <td className={styles.td}>
                                <button 
                                    className={`${styles.button} ${styles.viewUserButton}`}
                            ...
                            <td className={styles.td}>
                                <button 
                                    className={`${styles.button} ${styles.seeBooksButton}`}
                                    onClick={() => fetchOrderBooks(order.id)}
                                >
                                    See Books
                                </button>
                            </td>
            ...
            {isBooksModalOpen && (
                <div className={styles.modal}>
                    ...
                    <table className={styles.modalTable}>
                        <thead>
                            <tr>
                                ...
                                <th className={styles.modalTh}>Title</th>
                        ...
                        <tbody>
                            {orderBooks.map(book => (
                                <tr key={book.bookId}>
                                    ...
                                    <td className={styles.modalTd}>{book.title}</td>
                                    <td className={styles.modalTd}>
                                        <input 
                                            type="number" 
                                            value={book.quantity} 
                                            onChange={(e) => handleInputChange(e, book.bookId, 'quantity')}
                                            className={styles.input}
                                        />
                                    </td>
                    ...
                    <button onClick={() => setIsBooksModalOpen(false)} className={styles.closeButton}>Close</button>
                </div>
            )}
            ...
            {isUserModalOpen && (
                <div className={styles.modal}>
                    ...
                    <table className={styles.modalTable}>
                        ...
                        <tbody>
                            <tr>
                                <td className={styles.modalTd}>{userDetails.id}</td>
                                <td className={styles.modalTd}>{userDetails.name}</td>
                                <td className={styles.modalTd}>{userDetails.address}</td>
                            </tr>
                        </tbody>
                    </table>
                    <button onClick={() => setIsUserModalOpen(false)} className={styles.closeButton}>Close</button>
                </div>
            )}
        </div>
    );
